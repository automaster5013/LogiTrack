...
public class AlertPolicyService {
    ...
    public AlertPolicy resolve(String vehicleId){return policies.findByVehicleId(vehicleId).orElseGet(()->policies.findByVehicleId(AlertPolicy.DEFAULT_VEHICLE)
        .orElseThrow(()->new IllegalStateException("Default alert policy is missing")));}
    @Transactional(readOnly=true) public List<AlertPolicy> list(){return policies.findAllByOrderByVehicleIdAsc();}
