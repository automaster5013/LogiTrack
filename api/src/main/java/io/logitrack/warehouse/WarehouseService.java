...
public class WarehouseService {
 ...
 @Transactional public WarehouseTask receive(WarehouseCommand command,String key,String traceId){
  validate(command);var existing=tasks.findByIdempotencyKey(key);if(existing.isPresent())return existing.get();
 ...
 private void validate(WarehouseCommand c){if(c==null||blank(c.referenceNumber())||blank(c.warehouseId())||blank(c.sku())||c.quantity()<=0)throw new IllegalArgumentException("referenceNumber, warehouseId, sku and positive quantity are required");}
