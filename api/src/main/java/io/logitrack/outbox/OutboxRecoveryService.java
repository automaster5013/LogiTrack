...
public class OutboxRecoveryService {
    private final OutboxRepository events; private final OutboxRetryAuditRepository audits;
    public OutboxRecoveryService(OutboxRepository events,OutboxRetryAuditRepository audits){this.events=events;this.audits=audits;}
    ...
    @Transactional public FailureView retry(UUID id,String actor){
        var normalized=normalize(actor);var event=events.lockById(id).orElseThrow();event.retry();audits.save(new OutboxRetryAudit(id,normalized));return FailureView.from(event);
    }
