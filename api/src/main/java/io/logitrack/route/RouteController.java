...
public class RouteController {
    ...
    @GetMapping public List<RouteSnapshot> list(@RequestParam(required=false) List<UUID> deliveryIds,@RequestParam(defaultValue="200") int limit){
        ...
        if(deliveryIds==null)return repository.findAll(PageRequest.of(0,limit,
            Sort.by(Sort.Direction.DESC,"generatedAt").and(Sort.by(Sort.Direction.DESC,"id")))).getContent();
