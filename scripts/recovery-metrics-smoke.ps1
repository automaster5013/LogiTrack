...
foreach($name in @("LogiTrackApiDown","LogiTrackRecoveryMetricRefreshFailing","LogiTrackOutboxFailed","LogiTrackOutboxBacklogGrowing","LogiTrackDeadLetterBacklog")){
  if($names-notcontains $name){throw "Missing Prometheus rule: $name"}
}
Write-Host "PASS: recovery gauges exposed and 5 Prometheus alert rules loaded"
