...
try {
  ...
  do {
    ...
    $oldCount=(docker compose exec -T postgres psql -U logitrack -d logitrack -tAc "SELECT (SELECT count(*) FROM processed_events WHERE event_id='$oldProcessed')+(SELECT count(*) FROM outbox_events WHERE id='$oldOutbox')+(SELECT count(*) FROM outbox_retry_audits WHERE id='$oldOutboxAudit')+(SELECT count(*) FROM telemetry_points WHERE event_id='$oldTelemetry')").Trim()
  ...
  Write-Host "PASS: expired processed/outbox/audit/telemetry rows deleted, 3 recent rows retained, metrics exposed"
} finally {
