...
전체 운영 경계를 한 번에 읽기 전용으로 감사하려면 AWS CLI와 PowerShell이 설치된 관리자 환경에서 다음을 실행한다. 계정, instance·disk·보안 그룹, DNS, 자동 복구, snapshot, S3 backup, SSM 일정과 Budget 중 하나라도 기대값에서 벗어나면 즉시 실패한다.
