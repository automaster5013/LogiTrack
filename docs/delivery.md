...
- `.github/workflows/staging-health.yml`은 별도 cloud 자격 증명 없이 6시간마다 DNS, HTTP→HTTPS redirect, LogiTrack 로그인 화면 식별자, cache 금지된 runtime UUID endpoint, TLS 인증서의 14일 이상 잔여기간, CSP를 포함한 보안 header, 서버·프레임워크 식별 header와 내부 서비스 포트 비노출을 검사한다.
