...
GitHub 저장소의 실제 승인자·`main` 전용 deployment branch·environment 변수·staging secret 부재·정확히 하나인 repository secret 이름·기본 token read 권한과 action SHA 고정 설정은 `./scripts/github-cd-boundary-audit.ps1`로 읽기 전용 감사한다. 같은 감사는 Docker Hub 게시·provenance 감사 workflow가 활성 상태인지, 최신 `main` SHA의 성공 CI·게시·26시간 이내 provenance 감사 실행, 공개 repository 5개에 동일 SHA의 유효한 `linux/amd64` digest가 존재하는지도 확인한다. provenance 감사 artifact는 실행당 정확히 하나이며 main SHA·run attempt·repository 실행에 결속된 이름과 SHA-256 archive digest, 비어 있지 않은 크기, 미만료 상태, 30일 보존 기간, repository 범위 URL까지 검사한다. artifact를 임시 디렉터리에 내려받아 GitHub archive digest와 다시 대조하고, 정확한 tag·검증 JSON 10개와 `SHA256SUMS`만 존재하는지, 모든 내부 해시가 일치하는지, 각 tag의 main SHA·digest·platform과 SLSA 검증 subject/digest가 서로 같은지도 확인한 뒤 임시 파일을 제거한다. secret 값 자체는 GitHub API에서 다시 읽을 수 없으므로 이름과 실제 게시 성공을 결합해 구성과 기능을 검증한다.
