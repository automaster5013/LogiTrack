...
각 Docker Hub digest에는 GitHub OIDC의 단기 신원과 공개 Sigstore를 사용하는 SLSA build provenance attestation을 `actions/attest`로 발급해 registry에 함께 게시한다. 장기 서명 키는 저장하지 않으며, action은 전체 commit SHA로 고정한다. 게시 직후 `gh attestation verify`가 Docker Hub의 OCI bundle을 직접 읽어 GitHub OIDC issuer, 이 repository의 재사용 workflow, `refs/heads/main`, 정확한 source commit, GitHub-hosted runner와 SLSA provenance subject를 모두 검증한다. 다섯 attestation의 GitHub ID·검증 URL과 서비스별 검증 JSON·SHA-256을 release manifest의 해당 image에 결합하며, validator는 다른 repository의 URL, 잘못된 ID, 변조되거나 다른 digest를 가리키는 검증 결과를 거부한다. 이 단계에만 `id-token: write`, `attestations: write`, `artifact-metadata: write` 권한을 부여하며 PR에서는 호출 job 전체가 실행되지 않는다.
