...
각 Docker Hub digest에는 GitHub OIDC의 단기 신원과 공개 Sigstore를 사용하는 SLSA build provenance attestation을 `actions/attest`로 발급해 registry에 함께 게시한다. 장기 서명 키는 저장하지 않으며, action은 전체 commit SHA로 고정한다. 검증기는 runner의 가변 기본 도구 대신 버전과 공식 release archive SHA-256을 고정한 GitHub CLI를 HTTPS로 내려받아 일치 여부와 실행 버전을 확인한 뒤 사용한다. 게시 직후 `gh attestation verify`가 Docker Hub의 OCI bundle을 직접 읽어 GitHub OIDC issuer, 이 repository의 재사용 workflow, `refs/heads/main`, 정확한 source commit, GitHub-hosted runner와 SLSA provenance subject를 모두 검증한다. 검증기 버전·공식 source URL·archive SHA-256과 다섯 attestation의 GitHub ID·검증 URL, 서비스별 검증 JSON·SHA-256을 release manifest에 결합하며, validator는 다른 repository의 URL, 잘못된 ID, 검증기 drift, 변조되거나 다른 digest를 가리키는 검증 결과를 거부한다. 이 단계에만 `id-token: write`, `attestations: write`, `artifact-metadata: write` 권한을 부여하며 PR에서는 호출 job 전체가 실행되지 않는다.
...
GitHub 저장소의 실제 승인자·`main` 전용 deployment branch·environment 변수·staging secret 부재·정확히 하나인 repository secret 이름·기본 token read 권한과 action SHA 고정 설정은 `./scripts/github-cd-boundary-audit.ps1`로 읽기 전용 감사한다. 같은 감사는 Docker Hub 게시·provenance 감사 workflow가 활성 상태인지, 최신 `main` SHA의 성공 CI·게시·26시간 이내 provenance 감사 실행, 공개 repository 5개에 동일 SHA의 유효한 `linux/amd64` digest가 존재하는지도 확인한다. provenance 감사 artifact는 실행당 정확히 하나이며 main SHA·run attempt·repository 실행에 결속된 이름과 SHA-256 archive digest, 비어 있지 않은 크기, 미만료 상태, 30일 보존 기간, repository 범위 URL까지 검사한다. 다운로드 ZIP은 해제 전에 GitHub가 보고한 크기와 1 MiB archive 상한, 정확한 최상위 엔트리 11개, 중복·중첩 경로 부재, 엔트리별 압축 1 MiB·해제 2 MiB와 전체 해제 10 MiB 상한을 검증해 경로 탈출과 ZIP bomb를 차단한다. 그 뒤 임시 디렉터리에서 GitHub archive digest, 정확한 tag·검증 JSON 10개와 `SHA256SUMS`, 모든 내부 해시, 각 tag의 main SHA·digest·platform과 SLSA 검증 subject/digest 결속을 확인한다. 검증 JSON의 Sigstore issuer, signer workflow, source commit/ref, repository·owner ID, GitHub-hosted runner, CI build config와 원본 실행 URI도 현재 저장소 정책에 정확히 결속한다. 봉인된 서비스별 digest는 현재 Docker Hub SHA tag의 live digest와 다시 대조해 유효하지만 오래된 증거가 현재 상태를 대신하지 못하게 하고, 검증 후 임시 파일을 제거한다. secret 값 자체는 GitHub API에서 다시 읽을 수 없으므로 이름과 실제 게시 성공을 결합해 구성과 기능을 검증한다.
