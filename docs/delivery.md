...
publisher는 신규 빌드와 기존 ECR image 재사용 모두 Docker image metadata의 OS·architecture가 `linux/amd64`인지 확인한다. release manifest의 `imagePlatform`도 같은 값으로 고정해 이후 runtime 배포가 호환되지 않는 architecture를 암묵적으로 선택하지 못하게 한다. ECR에서 digest별 manifest media type도 다시 조회해 OCI image manifest 또는 Docker distribution v2 manifest만 허용하고, index나 legacy 형식은 배포 증적 생성 전에 거부한다.
