...
- GitHub 경고 각 페이지를 유효한 JSON 배열·최대 100건으로 제한해 성공 응답의 형식 drift나 비정상 대형 page를 정상 감사로 오인하지 않도록 차단
...
- provenance artifact를 GitHub 인증 302와 allowlist된 Azure Blob signed ZIP의 무인증 요청으로 분리해 Authorization 전달과 임의 redirect 다운로드 차단
- provenance ZIP entry를 Unix regular file로 제한하고 DOS directory·reparse 속성을 거부해 검증 후 추출 과정의 symlink·특수 파일 해석 차단
...
- OAuth state·nonce·PKCE verifier를 `__Host-` 쿠키로 고정해 부모 도메인 쿠키 주입과 경로별 쿠키 섀도잉 차단
- 인증 쿠키 helper의 path 기본값을 제거해 모든 신규 호출이 `__Host-` 계약에 맞는 경로를 명시하도록 강제
- DLQ replay smoke가 재격리 검증까지 끝난 뒤에만 성공을 출력하고 실패 경로에서도 주입한 poison·감사 데이터를 정리하도록 보강
...
- CSP 스타일 권한을 요소와 속성으로 분리해 `<style>` 주입은 nonce로 제한하고 지도·진행률의 동적 속성만 유지
- 원인이 연결된 API 검증·충돌 예외의 wrapper와 내부 상세를 외부 응답에서 제거하고 trace ID 기반 서버 로그에만 보존
- 인증 활성 환경의 공개 application origin 누락을 fail-closed로 전환해 Host 기반 redirect·CSRF origin 신뢰 fallback 차단
- 운영 API의 health 외 Actuator와 비정의 경로를 명시적으로 거부해 역할 없는 유효 토큰의 관리 표면 접근 차단
- API client ID의 공개 기본값을 제거하고 staging 웹·API가 동일한 필수 Cognito client ID를 사용하도록 배포 계약 고정
...
- DTO의 정수 필드에서 JSON 실수의 소수부를 버리는 축소 변환을 거부해 입력·검증·감사 데이터 간 수량 의미 불일치 차단
...
2026-09-26: 공개 저장소 첫 화면에 라이브 데모, 핵심 기능, 아키텍처 요약과 기술 스택을 배치해 프로젝트의 가치와 실행 경로를 빠르게 파악할 수 있도록 개선했다.
- 외부 기여를 위한 개발·검증·PR 절차, 행동강령, 구조화된 버그·기능 이슈 양식과 보안·질문 분리 경로를 추가
