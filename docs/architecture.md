...
HTTP 멱등 키는 PostgreSQL transaction advisory lock으로 키별 최초 생성을 직렬화하고 주문·배송·창고 명령의 원본 필드와 함께 검증한다. 같은 키와 동일 명령은 동시 요청에서도 기존 aggregate를 반환하지만, 같은 키에 다른 주문·차량·좌표·재고 명령을 보내면 409로 거부해 키 충돌이 데이터를 조용히 덮지 않게 한다.
