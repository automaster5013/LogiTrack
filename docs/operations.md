...
- 경고 임계값: `GET /api/alert-policies`에서 전역(`*`)·차량별 정책을 조회하고 `POST /api/alert-policies`와 필수 `X-Operator`로 저장한다. 차량별 정책이 없으면 전역값을 사용하며 `CLOSE < OPEN ≤ CRITICAL` 순서를 API와 DB가 모두 검증한다. `GET /api/alert-policies/audits`는 최근 50개 변경 snapshot을 반환한다. `./scripts/alert-policy-smoke.ps1`는 높은 임계값의 억제와 변경 즉시 경고 발생, 감사 2건을 검증한다.
