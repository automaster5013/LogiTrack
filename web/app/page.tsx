...
export default function Home(){
 ...
 return <main><header className="appHeader"><button className="brand" type="button" onClick={()=>setWorkspace("overview")}><span>LT</span><div><p className="eyebrow">OPERATIONS / LIVE</p><h1>LogiTrack</h1></div></button><div className="headerStatus"><span className={`signal ${connected?"on":""}`}><i/>{connected?"LIVE STREAM":"RECONNECTING"}</span><small>CONTROL TOWER</small></div></header>
  <nav className="workspaceNav" aria-label="Control tower workspaces">
   {(Object.keys(workspaceCopy) as Workspace[]).map(key=><button key={key} type="button" aria-current={workspace===key?"page":undefined} onClick={()=>setWorkspace(key)}><span>{workspaceCopy[key].label}</span>{key==="overview"&&activeAlerts.length>0&&<b>{activeAlerts.length}</b>}{key==="recovery"&&deadLetterTotal>0&&<b>{deadLetterTotal}</b>}</button>)}
  </nav>
  <section className="workspaceIntro"><div><p className="eyebrow">{copy.eyebrow}</p><h2>{copy.title}</h2><p>{copy.description}</p></div>{workspace!=="overview"&&<button type="button" onClick={()=>setWorkspace("overview")}>← 상황판으로</button>}</section>
  ...
  {workspace==="settings"&&<AlertPolicyPanel policies={policies} audits={policyAudits} deliveries={items} busy={policyBusy} onSave={savePolicy} onReset={resetPolicy} onRestore={restorePolicy}/>}
 </main>
}
