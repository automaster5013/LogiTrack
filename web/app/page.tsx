...
export default function Home(){
 ...
 async function savePolicy(policy:PolicyInput){setPolicyBusy(true);clearWorkspaceError("settings");try{const response=await fetch(`${API}/api/alert-policies`,{method:"POST",headers:{"Content-Type":"application/json","X-Operator":"control-tower"},body:JSON.stringify(policy)});if(!response.ok)throw new Error();await loadPolicies()}catch{setWorkspaceError("settings","경고 정책 저장에 실패했습니다. CLOSE < OPEN ≤ CRITICAL 순서를 확인해 주세요.")}finally{setPolicyBusy(false)}}
