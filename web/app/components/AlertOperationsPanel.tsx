...
export default function AlertOperationsPanel({alerts,deliveries,busyId,onSelect,onAcknowledge}:Props){
  ...
  useEffect(()=>{
    setVisibleCount(8);
  },[acknowledgement,query,scope,severity]);
  ...
  return <section className="alertBoard">
    ...
    <div className="alertGrid">{scopedAlerts.length===0?<div className="alertEmpty">{query||severity!=="ALL"||acknowledgement!=="ALL"?"현재 범위와 검색 조건에 맞는 경고가 없습니다.":scope==="ACTIVE"?"현재 대응이 필요한 경고가 없습니다. 해결된 경고는 ‘전체 이력’에서 확인할 수 있습니다.":"감지된 지연 또는 경로 이탈 이력이 없습니다."}</div>:visibleAlerts.map(alert=>{
      const delivery=deliveries.find(item=>item.id===alert.deliveryId);
