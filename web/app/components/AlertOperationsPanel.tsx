...
const formatAlertComparison=(alert:DeliveryAlert)=>{
  const difference=alert.observedValue-alert.thresholdValue;
  if(difference>0)return `기준 초과 +${formatAlertValue(difference,alert.alertType)} (${Math.round(alertExceedanceRatio(alert)*100).toLocaleString("ko-KR")}%)`;
...
export default function AlertOperationsPanel({alerts,deliveries,busyId,onSelect,onAcknowledge}:Props){
  ...
  const scopedAlerts=useMemo(()=>{
    ...
    return alerts.filter(alert=>{
      ...
      const searchableValues=[delivery?.vehicleId||"",delivery?.orderNumber||"",delivery?.originName||"",delivery?.destinationName||"",alert.message,alertTypeLabel[alert.alertType],alertSeverityLabel[alert.severity],alertStatusLabel[alert.status],alert.acknowledgedAt?"확인 완료":"미확인",alert.occurrenceCount>1?"반복 감지":"단일 감지",alert.acknowledgedBy||""];
      return (scope==="ALL"||alert.status==="ACTIVE")&&(severity==="ALL"||alert.severity===severity)&&(alertType==="ALL"||alert.alertType===alertType)&&matchesAcknowledgement&&(occurrence==="ALL"||alert.occurrenceCount>1)&&(!normalizedQuery||searchableValues.some(value=>value.toLowerCase().includes(normalizedQuery)));
    }).sort((left,right)=>{
  ...
  return <section className="alertBoard">
    <div className="alertHeader"><div><p className="eyebrow">예외 상황 관리</p><h2>배송 경고</h2></div>
      <div className="alertHeaderMeta"><div className="alertHeaderStats"><span><b>{active.length}</b><small>대응 필요</small></span><span><b>{unacknowledged.length}</b><small>미확인</small></span></div>
        ...
        <label className="alertFilter" htmlFor="alertSort"><span>정렬</span><select id="alertSort" value={sort} onChange={event=>setSort(event.target.value as "PRIORITY"|"RECENT"|"LONGEST"|"FREQUENT"|"EXCEEDANCE")}><option value="PRIORITY">대응 우선순위</option><option value="EXCEEDANCE">현재 초과율순</option><option value="RECENT">최근 감지순</option><option value="LONGEST">지속 시간순</option><option value="FREQUENT">감지 횟수순</option></select></label>
        <label className="alertSearch" htmlFor="alertSearch"><span>경고 검색</span><input id="alertSearch" type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="차량 · 주문 · 지역 · 상태 · 담당자"/></label>
