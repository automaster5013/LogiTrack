...
export default function AlertPolicyPanel({policies,audits,deliveries,busy,onSave,onReset,onRestore}:Props){
  ...
  const resetPolicy=()=>{
    if(window.confirm(`${vehicleId} 전용 정책을 삭제하고 전체 차량 기본값을 사용하시겠습니까? 이 작업은 변경 이력에 기록됩니다.`)) void onReset(vehicleId);
  };
  const restorePolicy=(audit:AlertPolicyAudit)=>{
    const target=audit.vehicleId==="*"?"전체 차량":audit.vehicleId;
    if(window.confirm(`${target} 정책을 ${new Date(audit.occurredAt).toLocaleString("ko-KR")} 시점의 값으로 복원하시겠습니까? 복원 작업도 변경 이력에 기록됩니다.`)) void onRestore(audit.id);
  };
  ...
  return <section className="policyBoard">
    ...
    <div className="policyBody"><form onSubmit={submit}>
      ...
      <p className="policyRule">임계값 순서: 해제 &lt; 경고 ≤ 긴급. 변경 사항은 다음 차량 위치 정보부터 적용됩니다. 기본값 전환과 이력 복원은 영향 확인 후 실행됩니다.</p>
      <div className="policyActions"><button disabled={busy||!selected}>{busy?"정책 저장 중…":"변경 이력과 함께 저장"}</button>
        {vehicleId!=="*"&&overridden&&<button type="button" className="policyReset" disabled={busy} onClick={resetPolicy} aria-label={`${vehicleId} 전용 정책을 전체 차량 기본값으로 전환`}>전체 차량 기본값 사용</button>}</div>
