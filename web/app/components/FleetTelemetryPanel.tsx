...
type Props = {
  ...
  selectedId?: string;
  scope: "LIVE" | "ATTENTION" | "STALE" | "ALL";
  query: string;
  ...
  onScopeChange: (scope: "LIVE" | "ATTENTION" | "STALE" | "ALL") => void;
...
function telemetryFreshness(delivery:Delivery,point?:TelemetryPoint){
  const timestamps=[delivery.lastTelemetryAt,point?.occurredAt].filter((value):value is string=>Boolean(value)).map(value=>new Date(value).getTime()).filter(value=>!Number.isNaN(value));
  if(!timestamps.length)return {label:"위치 이벤트 없음",stale:true};
  const occurredAt=new Date(Math.max(...timestamps));
...
export default function FleetTelemetryPanel({deliveries,telemetry,activeAlerts,selectedId,scope,query,liveDeliveries,attentionDeliveries,staleDeliveries,totalDeliveries,onSelect,onScopeChange,onQueryChange}:Props){
