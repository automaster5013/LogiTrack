...
export default function ReplayOperationsPanel({ events, totalEvents, audits, busyId, pageBusy, discardPlan, discardPlanBusy, onReplay, onDiscard, onLoadMore, onPrepareDiscard, onExecuteDiscard, onResetDiscardPlan }: Props) {
  ...
  return <section className="replayBoard">
    <div className="replayHeader"><div><p className="eyebrow">RECOVERY / DEAD LETTER QUEUE</p><h2>Selective event disposition</h2></div><div><b>{totalEvents}</b><span>PENDING · {pending.length} LOADED</span></div></div>
    <div className="replayColumns">
      <div className="deadLetters"><h4>QUARANTINED EVENTS</h4>
        ...
        {events.length<totalEvents&&<button className="loadMoreDlq" disabled={pageBusy} onClick={onLoadMore}>{pageBusy?"LOADING…":`LOAD ${Math.min(100,totalEvents-events.length)} OLDER EVENTS`}</button>}
