...
주문 조회: `GET /api/orders`, 배송 목록·단건 조회: `GET /api/deliveries`, `GET /api/deliveries/{id}`, 실시간 스트림: `GET /api/stream/deliveries`. 목록과 범위 없는 경로 조회는 최신 200건이 기본이며 `limit=1..500`으로 조정합니다. 기존 `POST /api/deliveries`는 호환성을 위해 유지하지만 신규 운영 흐름은 주문 생성 후 배차를 사용합니다.
