...
ECR repository 5개, Terraform destroy 차단, 최신 image 30개·미태그 7일 기본 보존 정책, 최소 권한 OIDC publisher role의 사전 구성은 `infra/aws/bootstrap` Terraform root에 정의되어 있습니다. CI는 format·provider 초기화·validate와 별도 trust-boundary smoke를 실행하지만, AWS 비용·리전·원격 state가 승인되기 전에는 plan/apply하지 않습니다.
